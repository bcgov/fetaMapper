...
hex.int.fc<-st_intersection(hex.grid, fisher.density[fisher.density$fisher_cap %in% c('very high', 'high','med', 'low', 'rare'),])
...
#Harvest locations year
if(FALSE){
  layer<-getSpatialQuery("SELECT shape FROM public.veg_comp_lyr_r1_poly2020 where proj_age_1 <= 12 and proj_age_1 >= 0 and bec_zone_code in ('SBS', 'IDF', 'MS', 'SBPS', 'BWBS')")
  layer$openess<-1
...
st_write(fish.constraints0, "feta_v0.shp", overwrite= T)
```

##### summaries by different boundaries
```{r, admin_bounds}
fish.constraints0<-sf::st_read("feta_v0.shp")
...
fish.constraints.boreal<-fish.constraints0[boreal,]
fish.constraints.columbia<-fish.constraints0[columbia,]
...
fish.constraints1$nfish_cat<-round(fish.constraints1$nfish,2)
fish.constraints1$geometry <- NULL
...
c_table<-fish.constraints.columbia[, sum(), by = nfish_cat]
write.csv(c_table, "ctable.csv")
b_table<-fish.constraints.boreal[, lapply(.SD, sum, na.rm=TRUE), by = nfish_cat]
...
#Get tfl boundary
tfl<-getSpatialQuery("SeLECT * from tfl_all_polygon")
tfl_columbia<-st_intersects(tfl, st_transform(fish.constraints.columbia, 3005))
names(tfl_columbia)<-tfl$ogc_fid
tfl.columbia<-stack(tfl_columbia)
setnames(tfl.columbia, c("values", "ind"), c("fid", "ogc_fid"))
...
names(tfl_boreal)<-tfl$ogc_fid
tfl.boreal<-stack(tfl_boreal)
setnames(tfl.boreal, c("values", "ind"), c("fid", "ogc_fid"))
...
#Get tsa boundary
tsa<-getSpatialQuery("SeLECT * from fadm_tsa_polygon")
tsa_columbia<-st_intersects(tsa, st_transform(fish.constraints.columbia, 3005))
names(tsa_columbia)<-tsa$ogc_fid
tsa.columbia<-stack(tsa_columbia)
setnames(tsa.columbia, c("values", "ind"), c("fid", "ogc_fid"))
...
names(tsa_boreal)<-tsa$ogc_fid
tsa.boreal<-stack(tsa_boreal)
setnames(tsa.boreal, c("values", "ind"), c("fid", "ogc_fid"))
...
aoi.boreal<-rbindlist(list(tsa.boreal,tfl.boreal, fn.boreal))
usethis::use_data(aoi.boreal)
aoi.columbia<-rbindlist(list(tsa.columbia,tfl.columbia, fn.columbia))
