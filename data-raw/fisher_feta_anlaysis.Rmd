...
writeRaster(ogma.ras, "ogma.tif",overwrite=TRUE)
writeRaster(defer.ras, "defer.tif",overwrite=TRUE)
...
rest.cavity.sbs.dry<-getTableQuery("select feature_id from public.veg_comp_lyr_r1_poly2020 where (SPECIES_CD_1 LIKE 'A%'  or SPECIES_CD_2 LIKE 'A%' or SPECIES_CD_3 LIKE 'A%') and CROWN_CLOSURE>=25 and QUAD_DIAM_125>=30 and BASAL_AREA>=32 and PROJ_HEIGHT_1>=35 and bec_subzone in ('dw','dh','dk');")
...
rest.cwd.sbs.dry<-getTableQuery("select feature_id from public.veg_comp_lyr_r1_poly2020 where PROJ_AGE_1>=135 and QUAD_DIAM_125>=22.7 and PROJ_HEIGHT_1>=23.7 and bec_zone_code = 'SBS' and bec_subzone in ('dw','dh','dk')")
...
#Fishers use BWBSdk, BWBSmw, BWBSwk, IDFdk3 &dk4, MSdc, MSdk, MSdm, MSdv, MSxk, MSxv, SBPSdc, SBPSmc, SBPSmk, SBPSxc, SBSdh, SBSdk, SBSdw, SBSmc, SBSmh, SBSmk, SBSmm, SBSmw, SBSwk
...
#st_write(fisher.distribution, "test.shp")
...
#Clear some mem
rm(movement.boreal, movement.dry.for, movement.sbs.dry, movement.poly, movement.sbs.wet, rest.cavity, rest.cavity.poly, rest.cavity.ras, rest.cavity.sbs.dry, rest.cavity.sbs.wet, rest.cwd, rest.cwd.boreal, rest.cwd.dry.for, rest.cwd.poly, rest.cwd.ras, rest.cwd.sbs.dry, rest.cwd.sbs.wet, rest.rust, rest.rust.boreal, rest.rust.dry.for, rest.rust.poly, rest.rust.ras, rest.rust.sbs.dry, rest.rust.sbs.wet)
gc()
...
if(FALSE){
   ...
   perm_Wetland<-getSpatialQuery("SELECT 1 as wetland, shape as geom FROM veg_comp_lyr_r1_poly2020 
...
OR (bclcs_level_3 = 'W' and (bclcs_level_5='SP' or bclcs_level_5='OP'))")
  perm_Wetland.ras <-fasterize::fasterize(perm_Wetland, prov.rast, field = "wetland")
    
  wetland<-getSpatialQuery("SELECT 1 as wetland, wkb_geometry from perm_wetland")
  wetland.ras <-fasterize::fasterize(wetland, prov.rast, field = "wetland")
  
  ebm_wetland<-getSpatialQuery("SELECT 1 as wetland, wkb_geometry from ebm_wetlnd_polygon")
  ebm_wetland.ras <-fasterize::fasterize(ebm_wetland, prov.rast, field = "wetland") 
  
  ebm_wetland.ras[is.na(ebm_wetland.ras[])]<-0
  wetland.ras[is.na(wetland.ras[])]<-0
  #perm_Wetland.ras[is.na(perm_Wetland.ras[])]<-0
  wtland<-wetland.ras+ebm_wetland.ras
  
  wtland[wtland[] > 1]<-1
  writeRaster(wtland, file="wetland.tif", format="GTiff", overwrite=TRUE)
}
###Get age
#Harvest locations year
if(FALSE){
  layer<-getSpatialQuery("SELECT shape FROM public.veg_comp_lyr_r1_poly2020 where proj_age_1 <= 12 and proj_age_1 >= 0 and (zone = 'BWBS' and subzone in ('dk','mw','wk', 'mk')) OR (zone = 'ESSF' and subzone in('xv','xcp','xc','mk', 'mc')) OR (zone = 'ICH' and subzone in('mm','dk')) OR (zone = 'IDF' and ((subzone = 'dk' and variant in ('1','2','3','4')) OR (subzone = 'mw' and variant in ('2')) OR (subzone = 'ww'))) OR  (zone = 'MS' and subzone in('dc','dk','dm','dv','xk','xv', 'un')) OR (zone = 'SBS' and subzone in('dh','dk','dw','mc','mh','mk','mm','mw','wk', 'vk', 'un')) OR (zone = 'SBPS' and subzone in('dc','mc','mk','xc')) OR (zone = 'ICH' and subzone = 'mc' and variant in('1','2'))
                                     OR (zone = 'CWH' and subzone = 'ws' and variant in('1','2'));")
  layer$openess<-1
...
hex.grid$openess_max<- exactextractr::exact_extract(wetlands.ras,hex.grid,c('sum'))
...
out4<-merge( hab_crit,out3, by.x = 'fid', by.y = 'fid', all.x = TRUE)

#out4$hab_chr<-as.integer(out4$hab_chr)
out4$nfish<-round(out4$nfish, 4)
out4$p_occ<-exp(-0.219*((out4$openess/3000)*100))/(1+exp(-0.219*((out4$openess/3000)*100)))/0.5
out4$p_occ_max<-exp(-0.219*((out4$openess_max/3000)*100))/(1+exp(-0.219*((out4$openess_max/3000)*100)))/0.5

#calculate abundance
out4$abund<-out4$nfish*out4$p_occ
out4$abund_pot<-out4$nfish*out4$p_occ_max
sum(out4$abund)

#keycol <-c("n_fish", "hab_chr","p_occ")
#setorderv(out4, keycol, order=c(-1,-1,-1))
hex.fish.dens<-st_intersects(fisher.distribution, hex.grid)

fish.constraints0<-merge( hex.grid[1:2],out4, by.x = 'fid', by.y = 'fid', all.x = TRUE)
test<-unique(unlist(hex.fish.dens))
fish.constraints1<-fish.constraints0[test,]
fish.constraints1<-st_as_sf(fish.constraints1)
st_write(fish.constraints1, "feta_v0.shp", overwrite= T, append =F)

```

###Calculate d2
```{r, d2}
fish.constraints1<-sf::st_read("feta_v0.shp")
#get the fisher population
fetaid<-data.table( fid = fasterize::fasterize(fish.constraints1, prov.rast, field = "fid")[], pop = raster("movement_p.tif")[])
fetaid<-fetaid[pop>0,]
freq_fetaid<-fetaid[ ,.(count = .N), by = c("fid", "pop")]
feta_pop<-freq_fetaid[freq_fetaid[, .I[which.max(count)], by = fid]$V1]

#get table of information
d2_est<-fish.constraints1 %>% st_drop_geometry()
d2_est<-data.table(d2_est)
d2_est<-merge(d2_est, feta_pop, by.y = "fid", by.x = "fid")

#get the cov
fisher.d2.cov <- list(matrix(c(0.5,	2.7,	0.6,	3.2,	-6.5, 2.7,	82.7,	4.9,	83.3,	-75.8, 0.6,	4.9,	0.9,	4,	-7.1, 3.2,	83.3,	4,	101.3,	-100.4, -6.5,	-75.8,	-7.1,	-100.4,	156.2), ncol =5, nrow =5),
...
d2_est[, denning:= (hab_den/3000)*100]
d2_est[, cavity:= (hab_cav/3000)*100]
d2_est[, rust:= (hab_rus/3000)*100]
d2_est[, cwd:= (hab_cwd/3000)*100]
d2_est[, mov:= (hab_mov/3000)*100]

d2_est[ pop == 1 & denning >= 0, denning:=log(denning + 1)][ pop == 1 & cavity >= 0, cavity:=log(cavity + 1)]
d2_est[ pop == 2 & denning >= 0, denning:=log(denning + 1)]
d2_est[ pop >= 3 & rust >= 0, rust:=log(rust + 1)]
   
#-----Truncate at the center plus one st dev
stdev_pop1<-sqrt(diag(fisher.d2.cov[[1]]))
stdev_pop2<-sqrt(diag(fisher.d2.cov[[2]]))
stdev_pop3<-sqrt(diag(fisher.d2.cov[[3]]))
stdev_pop4<-sqrt(diag(fisher.d2.cov[[4]]))
    
d2_est[ pop == 1 & denning > 1.6 + stdev_pop1[1], denning := 1.6 + stdev_pop1[1]][ pop == 1 & rust > 36.2 + stdev_pop1[2], rust :=36.2 + stdev_pop1[2]][ pop == 1 & cavity > 0.7 + stdev_pop1[3], cavity :=0.7+ stdev_pop1[3]][ pop == 1 & cwd > 30.4+ stdev_pop1[4], cwd :=30.4+ stdev_pop1[4]][ pop == 1 & mov > 26.8+ stdev_pop1[5], mov :=26.8+ stdev_pop1[5]]
d2_est[ pop == 2 & denning > 1.2 + stdev_pop2[1], denning := 1.2 + stdev_pop2[1]][ pop == 2 & rust > 19.1 + stdev_pop2[2], rust :=19.1 + stdev_pop2[2]][ pop == 2 & cavity > 0.5 + stdev_pop2[3], cavity :=0.5+ stdev_pop2[3]][ pop == 2 & cwd > 10.2+ stdev_pop2[4], cwd :=10.2+ stdev_pop2[4]][ pop == 2 & mov > 33.1+ stdev_pop2[5], mov :=33.1+ stdev_pop2[5]]
d2_est[ pop %in% c(3,4) & denning > 2.3 + stdev_pop3[1], denning := 2.3 + stdev_pop3[1]][ pop %in% c(3,4) & rust > 1.6 +  stdev_pop3[2], rust :=1.6  + stdev_pop3[2]][ pop %in% c(3,4) & cwd > 10.8+ stdev_pop3[3], cwd :=10.8 + stdev_pop3[3]][ pop %in% c(3,4) & mov > 21.5+ stdev_pop3[4], mov :=21.5+ stdev_pop3[4]]
d2_est[ pop >= 5 & denning > 24  + stdev_pop4[1], denning:=24+ stdev_pop4[1] ][ pop >= 5 & rust > 2.2+ stdev_pop4[2], rust :=2.2+ stdev_pop4[2]][ pop >= 5 & cwd > 17.4 + stdev_pop4[3], cwd :=17.4+ stdev_pop4[3]][ pop >= 5 & mov > 56.2+ stdev_pop4[4], mov :=56.2+ stdev_pop4[4]]
    
    
    #-----D2
d2_est[ pop == 1, d2:= mahalanobis(d2_est[ pop == 1, c("denning", "rust", "cavity", "cwd", "mov")], c(1.6, 36.2, 0.7, 30.4, 26.8), cov = fisher.d2.cov[[1]])]
d2_est[ pop == 2, d2:= mahalanobis(d2_est[ pop == 2, c("denning", "rust", "cavity", "cwd", "mov")], c(1.16, 19.1, 0.4549, 8.869, 33.1), cov = fisher.d2.cov[[2]])]
d2_est[ pop %in% c(3,4), d2:= mahalanobis(d2_est[ pop %in% c(3,4), c("denning", "rust", "cwd", "mov")], c(2.3, 1.6, 10.8, 21.5), cov = fisher.d2.cov[[3]])]
d2_est[ pop >= 5, d2:= mahalanobis(d2_est[ pop >= 5, c("denning", "rust", "cwd", "mov")], c(24.0, 2.2, 17.4, 56.2), cov = fisher.d2.cov[[4]])]

fish.constraints2<-merge(fish.constraints1, d2_est[,c("fid", "d2")], by.x = "fid", by.y = "fid", all.x = TRUE)
st_write(fish.constraints2, "feta_v1.shp", overwrite= T, append =F)
```

##### summaries by different boundaries
```{r, admin_bounds}
fish.constraints1<-sf::st_read("feta_v1.shp")
#fish.constraints1[nfish >= 0.43,cap:= "vh"][nfish >= 0.3 & nfish < 0.43, cap:= "h"][nfish >= 0.18 & nfish < 0.3, cap:="m"][nfish >= 0.07 & nfish < 0.18, cap:= "l"][nfish < 0.07, cap:="r"]
fish.constraints1$n_feta<-1

#get hab zones
hab.units<-getSpatialQuery("select * from fisher_ci_boreal_units_fine")
boreal<-unique(unlist(st_intersects(hab.units[hab.units$pop_unit == 'Boreal',], fish.constraints1)))
#columbia<-unique(unlist(st_intersects(hab.units[!hab.units$pop_unit == 'Boreal',],fish.constraints1)))
#fish.constraints.boreal<-fish.constraints1[boreal,]
#fish.constraints.columbia<-fish.constraints1[columbia,]

#need to add some to boreal and take away from columbia
leftovers<-sf::st_read("left_overs.shp")
boreal2<-unique(unlist(st_intersects(leftovers, fish.constraints1)))
boreal<-c(boreal , boreal2)

columbia <- 1:18704
columbia<-columbia[!(columbia %in% boreal)]

fish.constraints.boreal<-fish.constraints1[boreal,]
fish.constraints.columbia<-fish.constraints1[columbia,]

st_write(fish.constraints.columbia, "test_columbia.shp", overwrite= T)
st_write(fish.constraints.boreal, "test_boreal.shp", overwrite= T)
```

#### AOI summaries
```{r , sum_tsa}
fn<-getSpatialQuery("SELECT boundary_name, geometry, objectid FROM consultative_boundary_2021_10_22;")
fn_columbia<-st_intersects(fn, st_transform(fish.constraints.columbia, 3005))
names(fn_columbia)<-fn$boundary_name
fn.columbia<-stack(fn_columbia)
setnames(fn.columbia, c("values", "ind"), c("fid", "aoi"))
fn.columbia$aoi_sub<-'FN'

fn_boreal<-st_intersects(fn, st_transform(fish.constraints.boreal, 3005))
names(fn_boreal)<-fn$boundary_name
fn.boreal<-stack(fn_boreal)
setnames(fn.boreal, c("values", "ind"), c("fid", "aoi"))
fn.boreal$aoi_sub<-'FN'

#Get tfl boundary
tfl<-getSpatialQuery("SeLECT for_fl_id, wkb_geometry from tfl_all_polygon;")
tfl_columbia<-st_intersects(tfl, st_transform(fish.constraints.columbia, 3005))
names(tfl_columbia)<-tfl$for_fl_id
tfl.columbia<-stack(tfl_columbia)
setnames(tfl.columbia, c("values", "ind"), c("fid", "aoi"))
#tfl.columbia$ogc_fid<-as.integer(tfl.columbia$ogc_fid)
#tfl.columbia<-merge(data.table(tfl.columbia),data.table(tfl)[,c("ogc_fid", "for_fl_id")], by.x = "ogc_fid", by.y = "ogc_fid", all.x =T)
#tfl.columbia<-tfl.columbia[,c("fid", "for_fl_id")]
#setnames(tfl.columbia, "for_fl_id", "aoi")
tfl.columbia$aoi_sub<-'TFL'

tfl_boreal<-st_intersects(tfl, st_transform(fish.constraints.boreal, 3005))
names(tfl_boreal)<-tfl$for_fl_id
tfl.boreal<-stack(tfl_boreal)
setnames(tfl.boreal, c("values", "ind"), c("fid", "aoi"))
#tfl.boreal$ogc_fid<-as.integer(tfl.boreal$ogc_fid)
#tfl.boreal<-merge(data.table(tfl.boreal),data.table(tfl)[,c("ogc_fid", "for_fl_id")], by.x = "ogc_fid", by.y = "ogc_fid", all.x =T)
#tfl.boreal<-tfl.boreal[,c("fid", "for_fl_id")]
#setnames(tfl.boreal, "for_fl_id", "aoi")
tfl.boreal$aoi_sub<-'TFL'

#Get tsa boundary
tsa<-getSpatialQuery("SeLECT wkb_geometry, tsnmbrdscr from fadm_tsa_polygon where tsb_number is null;")
tsa_columbia<-st_intersects(tsa, st_transform(fish.constraints.columbia, 3005))
names(tsa_columbia)<-tsa$tsnmbrdscr
tsa.columbia<-stack(tsa_columbia)
setnames(tsa.columbia, c("values", "ind"), c("fid", "aoi"))
#tsa.columbia$ogc_fid<-as.integer(tsa.columbia$ogc_fid)
#tsa.columbia<-merge(data.table(tsa.columbia),data.table(tsa)[,c("ogc_fid", "tsnmbrdscr","tsbnmbrdsc")], by.x = "ogc_fid", by.y = "ogc_fid", all.x =T)
#tsa.columbia<-tsa.columbia[!is.na(tsa.columbia$tsbnmbrdsc),]
#fix a few of the tsa
#tsa.columbia$tsbnmbrdsc<-stringr::str_replace_all(tsa.columbia$tsbnmbrdsc, "Fort St John TSA","Fort St. John TSA")
#tsa.columbia$test<-stringr::str_replace_all(tsa.columbia$tsbnmbrdsc, tsa.columbia$tsnmbrdscr,"")
#tsa.columbia$test<-stringr::str_replace_all(tsa.columbia$test, "Wms Lk TSA","")
#tsa.columbia$test<-stringr::str_replace_all(tsa.columbia$test, "100 Mile House","")
#tsa.columbia$test<-stringr::str_replace_all(tsa.columbia$test, "Mid-Coast TSA","")	
#tsa.columbia<-tsa.columbia[,c("fid", "tsnmbrdscr", "test")]
#setnames(tsa.columbia, c("tsnmbrdscr", "test"), c("aoi", "aoi_sub"))
tsa.columbia$aoi_sub<-'TSA'

tsa_boreal<-st_intersects(tsa, st_transform(fish.constraints.boreal, 3005))
names(tsa_boreal)<-tsa$tsnmbrdscr
tsa.boreal<-stack(tsa_boreal)
setnames(tsa.boreal, c("values", "ind"), c("fid", "aoi"))
#tsa.boreal$ogc_fid<-as.integer(tsa.boreal$ogc_fid)
#tsa.boreal<-merge(data.table(tsa.boreal),data.table(tsa)[,c("ogc_fid", "tsnmbrdscr","tsbnmbrdsc")], by.x = "ogc_fid", by.y = "ogc_fid", all.x =T)

#tsa.boreal<-tsa.boreal[!is.na(tsa.boreal$tsbnmbrdsc),]
#fix a few of the tsa
#tsa.boreal$tsbnmbrdsc<-stringr::str_replace_all(tsa.boreal$tsbnmbrdsc, "Fort St John TSA","Fort St. John TSA")
#tsa.boreal$test<-stringr::str_replace_all(tsa.boreal$tsbnmbrdsc, tsa.boreal$tsnmbrdscr,"")
#tsa.boreal<-tsa.boreal[,c("fid", "tsnmbrdscr", "test")]
#setnames(tsa.boreal, c("tsnmbrdscr", "test"), c("aoi", "aoi_sub"))
tsa.boreal$aoi_sub<-'TSA'

#Nat Districts
nat_dr<-getSpatialQuery("SeLECT district_n, wkb_geometry from nat_dr;")
nat_dr_columbia<-st_intersects(nat_dr, st_transform(fish.constraints.columbia, 3005))
names(nat_dr_columbia)<-nat_dr$district_n
nat_dr.columbia<-stack(nat_dr_columbia)
setnames(nat_dr.columbia, c("values", "ind"), c("fid", "aoi"))
nat_dr.columbia$aoi_sub<-'NRD'

nat_dr_boreal<-st_intersects(nat_dr, st_transform(fish.constraints.boreal, 3005))
names(nat_dr_boreal)<-nat_dr$district_n
nat_dr.boreal<-stack(nat_dr_boreal)
setnames(nat_dr.boreal, c("values", "ind"), c("fid", "aoi"))
nat_dr.boreal$aoi_sub<-'NRD'

## Combine into a table per each fisher population
aoi.boreal<-rbindlist(list(tsa.boreal,tfl.boreal, fn.boreal,nat_dr.boreal))
usethis::use_data(aoi.boreal,overwrite = TRUE)
aoi.columbia<-rbindlist(list(tsa.columbia,tfl.columbia, fn.columbia,nat_dr.columbia))
usethis::use_data(aoi.columbia,overwrite = TRUE)
fish.constraints.boreal<-st_transform(fish.constraints.boreal, 4326)
usethis::use_data(fish.constraints.boreal,overwrite = TRUE)
fish.constraints.columbia<-st_transform(fish.constraints.columbia, 4326)
usethis::use_data(fish.constraints.columbia,overwrite = TRUE)
