    ...
    switch(input$attribute_def,
           Overview = {
             tagList(
              ...
              HTML("<h3>Overview</h3><p>Fishers are territorial animals that require specific forest structures in their territories (aka home range) to meet their life history needs. Because each individual fisher will have a unique territory shape and location based upon local habitat conditions, we cannot identify the location of each fisher territory across British Columbia (BC). Instead, we use a spatial grid of hexagons with an area of 30 km<sup>2</sup> to represent a “Fisher Equivalent Territory Area” (FETA). A 30 km<sup>2</sup> size approximates the measured average home range size of fisher in BC (Rich Weir, pers. comm.) and are an effective means to estimate habitat characteristics and assess whether they meet fisher needs.</p><br>")
            )
            },
           abund = {
             tagList(
